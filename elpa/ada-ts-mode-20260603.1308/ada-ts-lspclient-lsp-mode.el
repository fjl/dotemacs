;;; ada-ts-lspclient-lsp-mode.el -- LSP client interface for lsp-mode -*- lexical-binding: t; -*-
...
(cl-defmethod ada-ts-lspclient-command-execute ((_client (eql lsp-mode)) command &rest arguments)
...
(cl-defmethod ada-ts-lspclient-command-supported-p ((_client (eql lsp-mode)) command)
  "Determine if Language Server supports COMMAND."
  (lsp-can-execute-command? command))

(cl-defmethod ada-ts-lspclient-document-id ((_client (eql lsp-mode)))
  "Determine document identifier of current buffer."
  (lsp-text-document-identifier))

(cl-defmethod ada-ts-lspclient-format-region ((_client (eql lsp-mode)) beg end)
  "Format region BEG to END using Language Server."
  (if (= (- end beg) (buffer-size))
      (lsp-format-buffer)
    (lsp-format-region beg end)))

(cl-defmethod ada-ts-lspclient-workspace-configuration ((_client (eql lsp-mode)) scope &optional false)
...
(defvar ada-ts-lspclient--lsp-workspace-extra-dirs-alist nil)

(cl-defmethod ada-ts-lspclient-workspace-dirs-add ((_client (eql lsp-mode)) dirs)
  ...
  (when-let* ((root (lsp--workspace-root (car (lsp-workspaces))))
    ...
    (setq dirs (seq-filter
                ...
                dirs))
    (setq ada-ts-lspclient--lsp-workspace-extra-dirs-alist
          (assoc-delete-all root ada-ts-lspclient--lsp-workspace-extra-dirs-alist))
    (when dirs
      (push (cons root dirs) ada-ts-lspclient--lsp-workspace-extra-dirs-alist))))

(cl-defmethod ada-ts-lspclient-workspace-root ((_client (eql lsp-mode)) path)
  "Determine workspace root for PATH."
  (when-let* ((root (lsp-workspace-root path)))
    (file-name-as-directory (expand-file-name root))))

(defun ada-ts-lspclient-lsp-mode--normalize (value)
  "Normalize VALUE using lists, property lists, etc."
  (cond ((hash-table-p value)
         (let ((plist))
           (maphash
            (lambda (key value)
              (setq value (ada-ts-lspclient-lsp-mode--normalize value))
              (when value
           ...
           plist))
        ((listp value)
         (seq-map #'ada-ts-lspclient-lsp-mode--normalize value))
        ((vectorp value)
         (append (seq-map #'ada-ts-lspclient-lsp-mode--normalize value) nil))
...
(defun ada-ts-lspclient-lsp-mode--initialized ()
  ...
  (when-let* ((workspace (car (lsp-workspaces)))
    ...
    (with-current-buffer buffer
      (run-hooks 'ada-ts-lspclient-session-hook))))

(add-hook 'ada-ts-lspclient-find-functions #'ada-ts-lspclient-lsp-mode-try)
(add-hook 'lsp-after-initialize-hook #'ada-ts-lspclient-lsp-mode--initialized)

(defvar ada-ts-lspclient-lsp-mode--library-folders-fn nil)

(defun ada-ts-lspclient-lsp-mode--extra-folders (workspace)
  "Find extra folders for WORKSPACE."
  (let (folders)
    (when ada-ts-lspclient-lsp-mode--library-folders-fn
      (setq folders (funcall ada-ts-lspclient-lsp-mode--library-folders-fn workspace)))
    (if-let* ((root (lsp--workspace-root workspace)))
        (append folders
                (cdr (assoc-string root ada-ts-lspclient--lsp-workspace-extra-dirs-alist)))
      folders)))

(with-eval-after-load 'lsp-ada
  (eval
   '(when-let* ((client (gethash 'ada-ls lsp-clients)))
      (setq ada-ts-lspclient-lsp-mode--library-folders-fn
